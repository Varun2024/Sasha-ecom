...
export default function Login() {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (result?.user) {
                // Using merge: true ensures we don't wipe out existing cart/wishlist on every login
                await setDoc(doc(db, 'users', result.user.uid), {
                    email,
                    lastLogin: new Date(),
                }, { merge: true });
                
                toast.success('Welcome back!', { position: "bottom-center", theme: "light" });
    ...
    const handleLoginGoogle = async () => {
        ...
        try {
            ...
            if (guser?.user) {
                ...
                toast.success('Login successful!', { position: "bottom-center", theme: "light" });
                setTimeout(() => navigate('/cart'), 1500);
            }
        } catch (err) {
    ...
    return (
        <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-4">
            <ToastContainer />
            <div className="w-full max-w-[450px] bg-white p-8 md:p-12 shadow-[0_4px_25px_rgba(0,0,0,0.05)] rounded-sm">
                {/* Brand Header */}
                <div className="text-center mb-10">
                    <h2 className="text-3xl font-light tracking-widest uppercase text-gray-900 mb-2">Login</h2>
                    <div className="h-[1px] w-12 bg-gray-400 mx-auto mb-6"></div>
                    <p className="text-gray-500 text-sm font-light">Please enter your details to sign in</p>
                </div>

                {error && (
                    <div className="mb-6 text-center text-xs tracking-wide text-red-500 uppercase border-b border-red-100 pb-2">
                        {error}
                    </div>
                )}
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <div className="relative">
                        ...
                        <button 
                            type="button" 
                            className="absolute right-0 top-3 text-[10px] uppercase tracking-tighter text-gray-400 hover:text-gray-900"
                        >
                            Forgot?
                        </button>
                    </div>
                ...
                <div className="mt-12 text-center border-t border-gray-50">
                    <p className="text-gray-500 text-[12px] font-light tracking-wide">
                        ...
                        <button
                            type="button"
                            className="ml-2 text-gray-900 font-normal underline underline-offset-4 hover:text-gray-600"
                            onClick={() => navigate('/register')}
                        >
                            Create an Account
                        </button>
